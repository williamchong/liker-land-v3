...
export default defineEventHandler(async (event) => {
  ...
  // Buffer, not the Uint8Array the builder returns: the cache TransformStream
  // and the Node write stream below are both typed on Buffer.
  const id3Tag = Buffer.from(buildID3v2Tag({
    title: '3ook.com TTS',
