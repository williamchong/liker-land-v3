  ...
  const audioByText = await mapWithConcurrency(uniqueTexts, concurrency, async (text) => {
    ...
    try {
      ...
      if (bucket && segment.cacheKey) {
        try {
          const file = bucket.file(segment.cacheKey)
          const [metadata] = await file.getMetadata()
          // Same check the endpoint applies, so a dictionary edit regenerates
          // here too. Its stamp refresh is a write, so it stays behind the flag.
          const isStale = isCachedTTSPronunciationStale({
            file: values['write-cache'] ? file : { setMetadata: async () => {} },
