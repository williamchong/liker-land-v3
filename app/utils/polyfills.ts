import { installUint8ArrayToHex } from './uint8-array-to-hex'
...
// Every realm that runs pdf.js needs these: app/plugins/polyfill.ts covers the
// main thread, app/workers/pdf-worker.ts the worker. pdfjs-dist calls
// Promise.try (Chromium 128) and Promise.withResolvers (Chromium 119).
export function installPolyfills() {
  ...
  // pdf.js routes every worker message through Promise.try, so a missing shim
  // here leaves getDocument() hanging rather than rejecting.
  if (!Promise.try) {
    Promise.try = function <T, A extends unknown[]>(fn: (...args: A) => T, ...args: A) {
