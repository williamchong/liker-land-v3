// Coalesce rapid setItem calls so a burst of cache writes (e.g., 100 books
...
// order stays correct. Client-only: touches window/document listeners.
export function createDebouncedStorage(base: Storage, delayMs = 300): Storage {
  ...
  function safeSetItem(key: string, value: string) {
    try {
      ...
      console.warn('[debounced-storage] failed to persist', key, error)
    }
  }

  // removeItem can throw in the same restricted-storage contexts as setItem.
  function safeRemoveItem(key: string) {
    try {
      base.removeItem(key)
    }
    catch (error) {
      console.warn('[debounced-storage] failed to remove', key, error)
    }
  }
